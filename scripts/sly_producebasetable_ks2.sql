...
select
	...
	count(1) over (partition by isnull(new_laestab,lea*10000+estab)) dups
into #ks2_y1
from public_data.pt.ks2final2018edited f
	left join public_data.organisation.predecessors p
...
select
	rectype,
	lea*10000 + estab orig_laestab, iclose, isnull(new_laestab,lea*10000+estab) laestab,
	...
	count(1) over (partition by isnull(new_laestab,lea*10000+estab)) dups
into #ks2_y2
from public_data.pt.ks2final2017edited f
	left join public_data.organisation.predecessors p
		on f.lea*10000+estab=old_laestab
where
	rectype in (1,5) and		-- NB: differs from special
...
from #ks2_y1 a
	inner join #base b on
		a.orig_laestab=b.laestab
where
	not exists (select * from #ks2_y1 c where c.laestab=b.laestab)
)
update a
set
	a.laestab=a.orig_laestab,
...
from #ks2_y2 a
	inner join #base b on
		a.orig_laestab=b.laestab
where
	not exists (select * from #ks2_y2 c where c.laestab=b.laestab)
)
update a
set
	a.laestab=a.orig_laestab,
...
select
	...
	cast([full time boys aged 5] as int)+cast([full time boys aged 6] as int)+cast([full time boys aged 7] as int)+		-- NB: this differs from KS4 code
	...
	cast([full time boys aged 10] as int)+cast([full-time girls aged 10] as int) as comp_pupils,
	...
	case when cast([full time boys aged 10] as int)>=2 or cast([full-time girls aged 10] as int)>=2 then 1 else 0 end as comp_cohorts,
